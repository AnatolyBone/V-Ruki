...
const EditListing = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (typedListing.user_id !== user.id) {
          logSecurityEvent(
            'edit_attempt_unauthorized',
            `User ${user.id} tried to edit listing ${id}`,
            user.id
          );
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewPreviews([...newPreviews, ...files.map(file => URL.createObjectURL(file))]);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      ...
      <form
        ...
        className="space-y-8 bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm"
      >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            ...
            {newPreviews.map((preview, index) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-blue-600 text-[10px] text-white text-center py-0.5">
                  Новое
                </div>
              </div>
            ))}
