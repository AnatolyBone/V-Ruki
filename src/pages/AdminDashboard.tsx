    ...
    <div className="container mx-auto px-4 py-12">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500">Очередь проверки на сегодня ({listings.length})</p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl mb-8 flex items-start gap-4">
        ...
        <div className="text-sm">
          ...
          <p className="text-amber-800 leading-relaxed">
            Внимательно проверяйте текст и изображения. **Запрещены**: любые виды оружия, наркотические вещества, 
            финансовые пирамиды, поддельные документы, услуги сексуального характера и контент 18+. 
            При обнаружении — отклоняйте объявление.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          {listings.map((listing: any) => (
            <div key={listing.id} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-6 hover:border-blue-200 transition-colors">
              {/* Image Preview */}
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <div className="text-2xl font-black text-blue-600 mb-2">{listing.price.toLocaleString()} ₽</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-bold text-gray-400 uppercase">Автор:</div>
                    <div className="text-sm font-bold text-gray-800">{listing.profiles?.full_name || 'Не указан'}</div>
